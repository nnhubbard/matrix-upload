...
* Copyright (C) 2013 Zed Said Studio
...
(function( $ ){
	...
	$.fn.matrixMultiFileUpload = function(options) {  
		...
		// Check to see if we have the required info
		if (defaults.assetBuilderId == '') {
			alert('You must enter an asset ID');
			return;
		} else if (defaults.assetBuilderUrl == '') {
			alert('You must enter an asset builder URL');
			return;
		} else if (defaults.swfUrl == '') {
			alert('You must enter the URL to the swfupload file');
			return;
		} else if (defaults.buttonImageUrl == '') {
			alert('You must enter the URL to the button image');
			return;
		}
		...
		return _this.each(function() {
			...
			var action = "AB_"+defaults.assetBuilderId+"_ASSET_BUILDER_ACTION";
			var type = "AB_"+defaults.assetBuilderId+"_ASSET_BUILDER_CREATE_TYPE";
